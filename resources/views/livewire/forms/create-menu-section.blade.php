<div class="max-w-lg mx-auto p-4 dark:bg-dark shadow-lg rounded-md">
    <form wire:submit.prevent="saveSection">
        <!-- Step 1: Create Section -->
        <h1 class="text-lg mb-4">This will be the menu section a user can click on to filter between menu items. You must
            also tag menu items to these sections</h1>
        <div class="mb-4">
            <label for="sectionName" class="block text-sm font-medium text-white">Section Name</label>
            <input type="text" id="sectionName" wire:model="sectionName"
                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm dark:text-white dark:bg-slate-900"
                style="@error('sectionName') border-color: red; @enderror" />
            @error('sectionName')
        ...
        {{-- <div class="mb-4">
            <div class="dark:bg-slate-900 form-check form-switch" style="@error('status') border-color: red; @enderror">
